...
try:
    # Get user's footprint history from file
    CALCULATIONS_FILE = 'calculations.json'
    footprints = []
    if os.path.exists(CALCULATIONS_FILE):
        with open(CALCULATIONS_FILE, 'r') as f:
            all_calculations = json.load(f)
            user_email = st.session_state.get('email')
            if user_email:
                footprints = [calc for calc in all_calculations if calc['user_email'] == user_email]
                # Sort by creation date, descending
                footprints.sort(key=lambda x: x['created_at'], reverse=True)
    ...
    if st.button("📄 " + t.get("download_report", "Download Detailed Report"), type="primary", use_container_width=True):
        ...
        # Convert dict to object for compatibility with get_report_download_link
        ...
        st.download_button(
            ...
            data=pdf_data,
            file_name=f"carbon_footprint_report_{latest['created_at']}.pdf",
